...
class DependentFilteredEntityController extends Controller
{
    ...
    public function options(Request $request)
    {
        ...
        if ($entity_inf['many_to_many']['active']) {
            ...
            $qb->andWhere('e.id IN (:results)')
                ->setParameter('results', $joinTableResults);
        } else {
            if ($entity_inf['grandparent_property']) {
                $qb->leftJoin('e.'.$entity_inf['parent_property'], 'parent')
        ...
        if (( $level = $request->get('level') )&& ( $levelProperty = $request->get('level_property') ) ){
            $qb->andWhere(sprintf('e.%s = :level', $levelProperty))
                ->setParameter('level', $level);
        }
        $qb->andWhere('e.id != :excluded_entity_id')
        ...
        foreach ($entity_inf['child_entity_filters'] as $key => $filter) {
            ...
            $qb->andWhere('e.'.$filter['property'].' '.$filter['sign'].' :'.$parameterName)
